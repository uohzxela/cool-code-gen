...
class assign extends Expression {
    ...
    public void code(PrintStream s, AbstractSymbol so, AbstractSymbol method, CgenClassTable cTable) {
        expr.code(s, so, method, cTable);
        Location loc = cTable.getLocation(so, method, name);
        cTable.emitStoreAcc(loc.pointer, loc.offset);   
    }
...
class static_dispatch extends Expression {
    ...
    public void code(PrintStream s, AbstractSymbol so, AbstractSymbol method, CgenClassTable cTable) {
        int nextLabel = cTable.getLabel();
        for(int i=0; i < actual.getLength(); i++) {
            Expression argExpr = (Expression) actual.getNth(i);
            argExpr.code(s, so, method, cTable);
            cTable.emitPushAccToStack();
        }
        expr.code(s, so, method, cTable);
        CgenSupport.emitBne(CgenSupport.ACC, CgenSupport.ZERO, nextLabel, s);
        cTable.emitLoadAccFilename();
        CgenSupport.emitLoadImm(CgenSupport.T1, 1, s);
        CgenSupport.emitJal("_dispatch_abort", s);
        CgenSupport.emitLabelDef(nextLabel, s);
        AbstractSymbol typeName = type_name;
        if (typeName == TreeConstants.SELF_TYPE) {
            typeName = so;
        }
        cTable.emitStaticDispatch(typeName, name);
    }
...
class dispatch extends Expression {
    ...
    public void code(PrintStream s, AbstractSymbol so, AbstractSymbol method, CgenClassTable cTable) {
        ...
        CgenSupport.emitJal("_dispatch_abort", s);
        CgenSupport.emitLabelDef(nextLabel, s);
        //System.out.printf("expr in %s: %s\n",so, expr.get_type());
        if (expr.get_type() != TreeConstants.SELF_TYPE) {
            cTable.emitDispatch(expr.get_type(), name);
        } else {
            cTable.emitDispatch(so, name);
        }
    }
...
class cond extends Expression {
    ...
    public void code(PrintStream s, AbstractSymbol so, AbstractSymbol method, CgenClassTable cTable) {
        int elseLabel = cTable.getLabel();
        int finishLabel = cTable.getLabel();
        pred.code(s, so, method, cTable);
        CgenSupport.emitLoad(CgenSupport.T1, 3, CgenSupport.ACC, s);
        CgenSupport.emitBeqz(CgenSupport.T1, elseLabel, s);
        then_exp.code(s, so, method, cTable);
        CgenSupport.emitBranch(finishLabel, s);
        CgenSupport.emitLabelDef(elseLabel, s);
        else_exp.code(s, so, method, cTable);
...
class loop extends Expression {
    ...
    public void code(PrintStream s, AbstractSymbol so, AbstractSymbol method, CgenClassTable cTable) {
        int loopLabel = cTable.getLabel();
        int finishLabel = cTable.getLabel();
        CgenSupport.emitLabelDef(loopLabel, s);
        pred.code(s, so, method, cTable);
        CgenSupport.emitLoad(CgenSupport.T1, 3, CgenSupport.ACC, s);
        CgenSupport.emitBeq(CgenSupport.T1, CgenSupport.ZERO, finishLabel, s);
        body.code(s, so, method, cTable);
...
class typcase extends Expression {
    ...
    public void code(PrintStream s, AbstractSymbol so, AbstractSymbol method, CgenClassTable cTable) {
        expr.code(s, so, method, cTable);
        ArrayList<Branch> sortedBranches = cTable.sortBranches(cases);
        int finishLabel = cTable.getLabel();
        int voidLabel = cTable.getLabel();
        CgenSupport.emitBne(CgenSupport.ACC, CgenSupport.ZERO, voidLabel, s);
        cTable.emitLoadAccFilename();
        CgenSupport.emitLoadImm(CgenSupport.T1, 1, s);
        ...
        for (Branch branch : sortedBranches) {
            ...
            CgenSupport.emitBlti(CgenSupport.T2, branch.staticClassTag, nextLabel, s);
            CgenSupport.emitBgti(CgenSupport.T2, branch.maxDynamicClassTag, nextLabel, s);
            cTable.enterTempScope();
            cTable.addId(branch.b.name, new Location(CgenSupport.FP, cTable.tempFPOffset));
            cTable.emitPushAccToStack();
            branch.b.expr.code(s, so, method, cTable);
            cTable.emitPopStack();
            cTable.exitTempScope();
            CgenSupport.emitBranch(finishLabel,s);
            CgenSupport.emitLabelDef(nextLabel, s);
        }
...
class let extends Expression {
    ...
    public void code(PrintStream s, AbstractSymbol so, AbstractSymbol method, CgenClassTable cTable) {
        ...
        if (init.get_type() == null) {
            cTable.emitDefaultObjValue(type_decl);
        } else {
            init.code(s, so, method, cTable);
        }
        cTable.emitPushAccToStack();
        cTable.addId(identifier, new Location(CgenSupport.FP, cTable.tempFPOffset));
        body.code(s, so, method, cTable);
        cTable.emitPopStack();
        cTable.exitTempScope();
    }
...
class plus extends Expression {
    ...
    public void code(PrintStream s, AbstractSymbol so, AbstractSymbol method, CgenClassTable cTable) {
        cTable.emitArithOperation(e1, e2, so, method, CgenSupport.ADD);
    }
...
class sub extends Expression {
    ...
    public void code(PrintStream s, AbstractSymbol so, AbstractSymbol method, CgenClassTable cTable) {
        cTable.emitArithOperation(e1, e2, so, method, CgenSupport.SUB);
    }
}
...
class mul extends Expression {
    ...
    public void code(PrintStream s, AbstractSymbol so, AbstractSymbol method, CgenClassTable cTable) {
        cTable.emitArithOperation(e1, e2, so, method, CgenSupport.MUL);
    }
}
...
class divide extends Expression {
    ...
    public void code(PrintStream s, AbstractSymbol so, AbstractSymbol method, CgenClassTable cTable) {
        cTable.emitArithOperation(e1, e2, so, method, CgenSupport.DIV);
    }
...
class lt extends Expression {
    ...
    public void code(PrintStream s, AbstractSymbol so, AbstractSymbol method, CgenClassTable cTable) {
        ...
        cTable.emitPushAccToStack();                                                       
        e2.code(s, so, method, cTable);                                                 
        CgenSupport.emitLoad(CgenSupport.T2, 3, CgenSupport.ACC, s);                  
        CgenSupport.emitLoad(CgenSupport.T1, 1, CgenSupport.SP, s);                  
        cTable.emitPopStack();
        cTable.emitLoadAccTrue();
        CgenSupport.emitBlt(CgenSupport.T1, CgenSupport.T2, nextLabel, s);
...
class eq extends Expression {
    ...
    public void code(PrintStream s, AbstractSymbol so, AbstractSymbol method, CgenClassTable cTable) {
        ...
        cTable.emitPopStack();
        cTable.emitLoadAccTrue();
        CgenSupport.emitBeq(CgenSupport.T1, CgenSupport.T2, nextLabel, s);
...
class leq extends Expression {
    ...
    public void code(PrintStream s, AbstractSymbol so, AbstractSymbol method, CgenClassTable cTable) {
        ...
        cTable.emitPopStack();
        cTable.emitLoadAccTrue();
        CgenSupport.emitBleq(CgenSupport.T1, CgenSupport.T2, nextLabel, s);
        cTable.emitLoadAccFalse();
        CgenSupport.emitLabelDef(nextLabel, s);
    }
}
...
class new_ extends Expression {
    ...
    public void code(PrintStream s, AbstractSymbol so, AbstractSymbol method, CgenClassTable cTable) {
        if (type_name == TreeConstants.SELF_TYPE) {
            ...
            CgenSupport.emitMove("$s1", CgenSupport.T1, s);
            CgenSupport.emitLoad(CgenSupport.ACC, 0, CgenSupport.T1, s);
            CgenSupport.emitJal("Object.copy", s);
            CgenSupport.emitLoad(CgenSupport.T1, 1, "$s1", s);
            CgenSupport.emitJalr(CgenSupport.T1, s);
            return;
        }
        CgenSupport.emitPartialLoadAddress(CgenSupport.ACC, s); CgenSupport.emitProtObjRef(type_name, s); s.println();// la $a0 <Class>_protObj
        CgenSupport.emitJal("Object.copy", s); // jal Object.copy
        CgenSupport.emitJal(type_name + CgenSupport.CLASSINIT_SUFFIX, s); // jal <Class>_init
    }
...
class object extends Expression {
    ...
    public void code(PrintStream s, AbstractSymbol so, AbstractSymbol method, CgenClassTable cTable) {
        //System.out.printf("object: %s, method: %s, type: %s\n", so, method, get_type());
        if (get_type() == TreeConstants.SELF_TYPE) {
            cTable.moveSelfToAcc();
        } else {
            Location loc = cTable.getLocation(so, method, name);
            cTable.emitLoadAcc(loc.pointer, loc.offset);
        }
    }
